...
if fl is not None:
    ...
    os.chdir(r"C:\Users\Sai\OneDrive\Documents\Desktop\tictaco\pyandstreamlit")
    df=pd.read_csv("Sample-Superstore",encoding="ISO-8859-1")


col1,col2=st.columns((2))
df["Order Date"]=pd.to_datetime(df["Order Date"])

#getting the min max data
...
if not region and not state and not city:
    ...
    filtered_df = df3[df3["State"].isin(region) & df3["State"].isin(state) & df3["City"].isin(city)]

categorydf=filtered_df.groupby(by=["Category"],as_index=False)["Sales"].sum()
with col1:
...
cl1,cl2=st.columns(2) #need 2 columns
with cl1:
...
with cl2:
    with st.expander("Region_ViewData"):
        ...
        csv=categorydf.to_csv(index=False).encode('utf-8')
        st.download_button("Download Data",data=csv,file_name="Reason.csv",mime="text/csv",
                           help="click here to download the data as a csv file")
...
with chart2:
    ...
    st.plotly_chart(fig,use_container=True)
#specific col data into  table format
...
with st.expander("View Data"):
    styled_filtered_df = filtered_df.iloc[:500, 1:20:2].style.background_gradient(cmap="Oranges")
    st.write(styled_filtered_df)

# Displaying the same styled DataFrame outside the expander
